...
class HardSimpleVFE(nn.Module):
    ...
    def forward(self, features, num_points, coors):
        ...
        points_mean = points.sum(dim=1) / num_points.type_as(points).view(-1, 1)  # (N, C)
        # 计算每个点到体素质心（初始均值）的距离
        ...
        # 计算加权平均
        weighted_sum = (points * weights.unsqueeze(-1)).sum(dim=1)           # (N, C)
        weight_norm = weights.sum(dim=1, keepdim=True)                        # (N, 1)
        weighted_mean = weighted_sum / weight_norm                            # (N, C)
